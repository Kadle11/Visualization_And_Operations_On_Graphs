...
void Display()
{
    ...
    for(int i=0;i<noOfEdges;i++)
    {
      ...
      {
        if(point[k].nodeNum == edgeWeights[i].node1)
        {
            node1x = point[k].x;
            node1y = point[k].y;
        }

        else if(point[k].nodeNum == edgeWeights[i].node2)
        {
            node2x = point[k].x;
            node2y = point[k].y;
        }
      ...
      //if graph is undirected an nonweighted
      if(flag.directed == 0 && flag.weighted == 0)
      {
        glColor3f(1,0,0);
        factor=0;
      }

      else if(flag.directed == 0 && flag.weighted == 1)
      {
        glColor3f(1,0,0);
        factor=factor+0.5;
      }
